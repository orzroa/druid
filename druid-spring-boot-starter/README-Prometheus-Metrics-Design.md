# Druid Prometheus 低基数监控方案

## 目标与边界

SQL /URI 文本是高基数数据；它们不得作为 Prometheus label，也不得为每个值创建 Meter。
本方案面向多服务、多实例部署，将 Prometheus 用于容量、趋势和告警，将明细定位交给独立的结构化日志。

```
SQL / URI 事件
  ├─ 固定数量的聚合 Meter ──> 应用已有 MeterRegistry ──> /actuator/prometheus
  └─ 明细（采样正常事件；全部异常事件） ──> logger: druid.metrics.event ──> 独立日志文件
```

这不是 exporter：starter 不增加 HTTP 端点、不创建独立 registry；只写入应用已有的 Micrometer `MeterRegistry`。
...
| --- | --- | --- | --- | --- |
| SQL 执行数及耗时 | `druid.sql.execution.duration` Timer | `{datasource}` | `druid_sql_execution_duration_seconds_count`、`_sum`、`_max` | 每一次 SQL execute，包括执行失败；`_count` 即 SQL 执行数 |
| 慢 SQL 数 | `druid.sql.slow` Counter | `{datasource}` | `druid_sql_slow_total` | 耗时达到 `slow-sql-millis` |
| 大读取 SQL 数 | `druid.sql.large.read` Counter | `{datasource}` | `druid_sql_large_read_total` | 每次 ResultSet 关闭时，读取行数达到 `large-sql-read-rows` 的 ResultSet 次数 |
| 大写入 SQL 数 | `druid.sql.large.write` Counter | `{datasource}` | `druid_sql_large_write_total` | 每个有效 update count 达到 `large-sql-write-rows` 的结果次数；batch 可产生多次 |
| URI 访问数及耗时 | `druid.uri.request.duration` Timer | `{}` | `druid_uri_request_duration_seconds_count`、`_sum`、`_max` | 每一个完成的同步 HTTP 请求；`_count` 即 URI 访问数 |
| 慢 URI 数 | `druid.uri.slow` Counter | `{}` | `druid_uri_slow_total` | 耗时达到 `slow-uri-millis` |
| 大读取 URI 数 | `druid.uri.large.read` Counter | `{}` | `druid_uri_large_read_total` | 请求累计读取行数达到 `large-uri-read-rows` |
| 大写入 URI 数 | `druid.uri.large.write` Counter | `{}` | `druid_uri_large_write_total` | 请求累计写入行数达到 `large-uri-write-rows` |
| 大量执行 SQL 的 URI 数 | `druid.uri.large.sql.executions` Counter | `{}` | `druid_uri_large_sql_executions_total` | 请求 SQL 次数达到 `large-uri-sql-executions` |
...
本方案不维护 SQL/URI 的长期明细缓存，也不使用 LRU。事件发生时只更新固定的聚合 Meter；日志异步队列最多保留 `queue-size` 个待写事件，实际内存取决于事件文本大小。正常事件依据采样率写日志，异常事件在正常运行条件下全量写日志；队列彻底满或日志系统故障等紧急情况下允许整条丢弃，但任何已输出事件都不截断 SQL/URI 内容。
...
# 当前文件与全部历史归档文件的总容量上限，超过后优先删除最旧归档
spring.datasource.druid.prometheus.logging.total-size-cap=1GB
# 应用关闭时等待异步日志队列刷盘的最长时间
spring.datasource.druid.prometheus.logging.shutdown-flush-timeout=3s
```

listener 始终注册，`prometheus.enabled=false` 时仅停止指标更新和事件日志输出；Apollo 改为 `true` 后，下一次事件立即恢复。事件处理路径每次重新读取 `enabled`、全部阈值和 `normal-sample-rate`，不能在初始化时复制为不可变的普通字段。固定 URI Meter 启动时注册；SQL Meter 按稳定的 `datasource` 懒注册。通过 `DruidPrometheusMetricsRefresher.refresh(...)` 或 Apollo/Spring 的 bean 刷新替换配置对象后，无须注销或重建已有 Meter。`normal-sample-rate` 必须在 `[0, 1]`；配置绑定时会截断到该范围。
...
阈值的比较语义为“达到即计入”（`>=`）。阈值为 `0` 时关闭对应的异常判断；负数属于非法配置，动态更新时拒绝该值并继续使用上一个有效值。正数按正常阈值处理，避免错误配置导致日志风暴。
...
```text
/opt/logs/druid-metrics-events.log
```
...
部署约定为每个服务实例运行在独立容器文件系统中，因此各实例的 `/opt/logs/druid-metrics-events.log` 不会被多个 JVM 同时写入。
...
1. listener 始终注册；URI Meter 启动时固定注册，SQL Meter 按稳定的 `datasource` 懒注册。共 9 种业务 Meter，不按 SQL/URI 创建 Meter。
2. SQL execute、update count、ResultSet close 和 Web 请求事件只更新聚合 Meter，并按阈值写日志。
3. SQL MD5、规整 SQL、URI 和原始 SQL 都不得用于 Prometheus 标签或进程内明细缓存；SQL MD5 和规整 SQL 只作为三类 SQL 日志字段使用。
4. 单元测试应验证不同 SQL 在同一 datasource 下命中相同 Meter、不同稳定 datasource 分别聚合、不同 URI 命中同一无标签 Meter、配置刷新在下一次事件生效，以及 Meter 数量不随 SQL/URI 数量增加。
5. 集成验收时连续发送大量不同 SQL/URI，确认 `/actuator/prometheus` 不出现 `sql=` 或 `uri=` 标签，仅 SQL 指标出现 `datasource` 标签；检查正常运行条件下异常事件全量进入独立日志。
6. Logback 自动配置测试应覆盖默认值、应用属性覆盖、Apollo 变更后原子重建、非 Logback 回退提示、INFO/WARN 队列丢弃策略，以及停机最多等待 `shutdown-flush-timeout`。
7. Starter 增加 optional 的 `logback-classic` 依赖，Logback 自动配置使用 `@ConditionalOnClass`；不得强制使用其他日志实现的应用引入 Logback。
8. 为支持 `prometheus.enabled` 从 `false` 动态恢复，自动配置不得使用该属性作为创建 listener Bean 的 `@ConditionalOnProperty`；listener 始终存在并在事件路径读取当前开关。
